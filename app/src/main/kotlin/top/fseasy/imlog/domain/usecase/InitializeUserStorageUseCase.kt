...
class InitializeUserStorageUseCase
@Inject
constructor(
  ...
  suspend operator fun invoke(
    ...
    storageRepository.setSharedStorageRootUriAndUpdateInitData(
        userId,
        determinedResult.rootUriString,
    )
  ...
  /**
   * After user select the storage uri, we should then determine use it directly or create a
   * subdirectory with default root name.
   */
  private suspend fun determineSharedStorageRootUri(
      userSelectedUriStr: UriStr,
  ) = runCatching {
    val dirName =
        storageRepository.getDisplayNameOrDefault(
            userSelectedUriStr,
