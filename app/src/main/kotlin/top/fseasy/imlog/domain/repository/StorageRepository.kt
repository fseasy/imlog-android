...
/**
 * StorageRepository: 1. thumbnail root dir 2. manage media-storage-root-uri 3. save message media
 * file NOTE: in the api, we use `String` instead of Uri to follow the clean rule.
 */
interface StorageRepository {

  /**
   * StorageRepo will hold the shared storage root uri for each user. save uri to user preference
   * and update flag field in app-init-data table.
   *
   ...
   */
  suspend fun resolveStoragePathToAbsolutePathsWithoutCreating(
   ...
   * No exception thrown.
   *
   * @param srcMimeType - it null, will resolve it internal for Uri type target. For condition where
   *   targetPath is InternalOnly, leave it to null as it's not needed
   *
  ...
  /**
   * No exception will be thrown. If path is invalid, return null. else return metadata that might
   * be inaccurate in edge condition where data is corrupted.
   *
   * Run in IO threads.
   */
  ...
  /**
   * No exception will be thrown. If uri invalid, return null. else return metadata that might be
   * inaccurate in edge condition where data is corrupted.
   *
   * Run in IO threads.
   */
   ...
   * Run in IO threads.
   */
  suspend fun getGenericFileMetadataOrNull(
