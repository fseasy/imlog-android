...
sealed interface ThumbnailScale {
  ...
  /**
   * Obey the input image's aspect and fit to the max-sized boundary without padding. Do nothing if
   * image's size already fit to the max-size
   */
  ...
  data class ScaleAndCroppingCenterToFill(val width: Int, val height: Int) : ThumbnailScale {
    ...
    /**
     * If any edge length of original image less than target size, we give up stretching by scale
     * the target size.
     *
     * else, the target size should be just kept the set value, leave the cropping logic do the left
     * things.
     */
    override fun calculateScaledWidthHeight(inputWidth: Int, inputHeight: Int): Pair<Int, Int> {
      if (inputWidth <= 0 || inputHeight <= 0) {
