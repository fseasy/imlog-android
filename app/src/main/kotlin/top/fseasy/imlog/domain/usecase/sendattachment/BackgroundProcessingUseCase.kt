...
constructor(
  ...
  /**
   * Should be called in background process (like Worker in Android) to make the following logic
   * running more stably
   */
  internal suspend operator fun invoke(
    ...
    val internalCacheFilePath =
        storagePathUseCase.buildMessageCacheFileStoragePath(
            userId = payload.userId,
            filename = payload.cacheFilename,
        )
    ...
    when (
        val result =
            copyFileUseCase.copyInternalCacheToSharedStorageAndUpdateState(
                messageId = messageId,
                userId = payload.userId,
                topicId = payload.topicId,
                messageTimestamp = payload.messageTimestamp,
    ...
    when (
        val result =
            finishProcessingUseCase.onSuccess(
                messageId,
                internalCachePathModel = internalCacheFilePath,
            )
    ) {
      is FinishProcessingStageResult.Failure ->
          return finishProcessingUseCase.onFailure(
              messageId = messageId,
