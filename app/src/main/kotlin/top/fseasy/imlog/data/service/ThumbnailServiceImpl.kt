...
constructor(
      ...
      withContext(Dispatchers.IO) {
        ...
        val requestBuilder =
            ImageRequest.Builder(context)
                ...
                .allowHardware(
                    false
                ) //  false to allow we get bytes from memory (can't get from GPU)
                .precision(Precision.EXACT) // Exact to avoid any fuzzy condition
                .scale(
                    Scale.FILL
                ) // Set to FILL is applicable to both Fit & Crop with our pre-calculated size
  ...
  override suspend fun generateVideoThumbnail(request: ThumbnailGenerateRequest): ByteArray =
      withContext(Dispatchers.IO) {
        ...
        FrameExtractor.Builder(context, mediaItem).setEffects(listOf(presentation)).build().use {
            extractor ->
