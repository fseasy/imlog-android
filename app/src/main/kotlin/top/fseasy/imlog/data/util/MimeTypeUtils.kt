...
object MimeTypeUtils {
  ...
  /**
   * Retrieves the MIME type of File, falling back to a default value if unresolved. No exception
   * will be thrown. Run in IO thread.
   */
  ...
  /**
   * Retrieves the MIME type of File, or null if it cannot be determined. No exception will be
   * thrown. Run in IO thread.
   */
  suspend fun getMimeTypeOrNull(file: File): String? =
      withContext(Dispatchers.IO) {
        if (!file.exists() || file.isDirectory) return@withContext null

        // 1. Resolve via file extension
        syncGetMimeTypeFromExtension(file.extension)?.let {
          return@withContext it
        }
        // 2. Resolve via file header bytes
        syncGetMimeTypeFromStream { FileInputStream(file) }
            ?.let {
              return@withContext it
            }
  ...
  /**
   * Retrieves the MIME Type of Uri, falling back to a default value if unresolved. No exception
   * will be thrown. Run in IO thread.
   */
  ...
  /**
   * Retrieves the MIME type of Uri, or null if it cannot be determined. No exception will be
   * thrown. Run in IO thread.
   */
      ...
      withContext(Dispatchers.IO) {
        ...
        if (mimeType == null || mimeType == DEFAULT_MIME_TYPE) {
          ...
          if (extMime != null) {
            ...
            val headerMime = syncGetMimeTypeFromStream {
  ...
  /**
   * Retrieves the MIME type of absolutePathModel, falling back to a default value if unresolved. No
   * exception will be thrown. Run in IO thread.
   */
  ...
  /**
   * Retrieves the MIME type of absolutePathModel, falling back to null if unresolved. No exception
   * will be thrown. Run in IO thread.
   */
  ...
  private fun syncGetMimeTypeFromExtension(extension: String): String? {
    ...
    return MimeTypeMap.getSingleton().getMimeTypeFromExtension(lowerExt)
        ?: syncGetFallbackMimeType(lowerExt)
  }
