...
object UriPathUtil {
            ...
            try {
              ...
              if (path != null) {
                ...
                FileDeleteResult.Error(
                    IllegalStateException("Uri [$uri] is invalid, no path found")
                )
              }
            } catch (e: CancellationException) {
              throw e
              ...
              FileDeleteResult.Error(
                  IllegalStateException("Uri [$uri] is invalid, neither `content://` nor `file://`")
              )
        }
      }

  /**
   * find for given subDirs / file paths. It **can't** distinguish if it's a file/dir. run in IO
   * thread
   */
  ...
  /**
   * find or create uri for the given relative file path. run in IO coroutine
   *
  ...
  /**
   * find or create uri for given sub Dirs. run in IO coroutine
   *
                    ...
                    DocumentsContract.createDocument(
                        ...
                        ?: return@withContext FindOrCreateFileUriResult.Error.Unexpected(
                            IllegalStateException(
                                "Failed to create ${if (isFile) "file" else "directory"}: $nodeName"
                            )
                        )
                  }
  ...
  /**
   * 生成 LRU 缓存的唯一标识 Key 格式示例: "content://com.android.externalstorage...::IMLog/2026-06/backup.txt"
   */
