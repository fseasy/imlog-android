...
object MediaDurationUtils {
  ...
  /**
   * Retrieves the duration of a media file (Audio or Video) in milliseconds. Returns 0 if
   * unresolved or if the file is invalid.
   *
   * Run in IO thread.
   */
   ...
   */
  suspend fun getDurationOrNull(file: File): Duration? =
  ...
  /**
   * Retrieves the duration of a media Uri (Audio or Video) in milliseconds. Returns 0 if unresolved
  ...
  /**
   * Retrieves the duration of a media Uri (Audio or Video), or null if unresolved. Will first try
   * to query system db info without reading file.
   *
  ...
  /**
   * Retrieves the duration of a media Uri (Audio or Video) by reading file (header or track). No
   * business exception will be thrown (but may throw CancellationException due to coroutine)
   */
  ...
  /**
   * Fast Path: Queries the system MediaStore provider database directly. Only works for
   * 'content://' Uris that expose a duration column.
   */
  ...
  private fun syncGetDurationFromRetriever(
